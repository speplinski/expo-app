...
class OakDSimulation:
    def __init__(self, horizontal_segments_count: int, vertical_segments_count: int, cameras_count: int):
        self.cameras_count = cameras_count
        self.distances = np.zeros((vertical_segments_count, horizontal_segments_count * cameras_count))
        self.visitors = []
    ...
    def get_distances(self) -> list[np.ndarray]:
        cameras_distances = []
        segment_width = self.distances.shape[1] // self.cameras_count

        for i in range(self.cameras_count):
            start_col = i * segment_width
            end_col = (i + 1) * segment_width
            camera_segment = self.distances[:, start_col:end_col].copy().flatten()
            cameras_distances.append(camera_segment)

        return cameras_distances
